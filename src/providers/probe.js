 ...
 * for. HAND-MAINTAINED — catalog.js is regenerated by
 * `bun run scripts/sync-providers.js` and anything written there is lost.
 *
 ...
 *
 *  1. `oauth.userInfoUrl` — from the generated catalog, refreshed by
 *     `scripts/sync-providers.js`, so it tracks the upstream registry.
 *  2. `apiKey.verifyUrl` — hand-maintained in byok.js, but purpose-built for
